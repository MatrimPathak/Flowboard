...
const app = new Hono()
	.use("/:taskId/attachments", async (c, next) => {
		const method = c.req.method;
		if (method === "POST") {
			c.req.raw.bodyLimit = MAX_FILE_SIZE + 1024 * 1024;
		}
		await next();
	})
	.get(
...
workspaceId,
	...
	.delete("/:taskId/worklogs/:worklogId", sessionMiddleware, async (c) => {
		...
		const updates: Record<string, unknown> = { timeSpent: newTimeSpent };
		if (currentRemaining !== null) {
			updates.remainingEstimate = Math.min(currentRemaining + deletedTimeSpent, taskData?.originalEstimate ?? currentRemaining + deletedTimeSpent);
		}
		await taskRef.update(updates);
