...
import { generateDocId, stripUndefinedFields } from "@/lib/docs-utils";
...
export async function createDocument(
  ...
  const now = Date.now();
  await setDoc(doc(scopedColl(workspaceId, projectId), id), stripUndefinedFields({
    ...payload,
    createdAt: now,
    updatedAt: now,
...
export async function updateDocument(
  ...
  await updateDoc(doc(scopedColl(workspaceId, projectId), id), stripUndefinedFields({ ...(rest as Partial<StoredFields>), updatedAt: Date.now() }));
}
